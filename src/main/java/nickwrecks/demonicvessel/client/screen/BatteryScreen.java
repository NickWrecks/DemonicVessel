...
public class BatteryScreen extends AbstractContainerScreen<BatteryMenu> {
    ...
    public boolean mouseClicked(double pMouseX, double pMouseY, int pButton) {
        if(pMouseX>leftPos-configOffsetLeft && pMouseX<=leftPos-configOffsetLeft+16) {
            if(pMouseY>topPos+configOffsetTop && pMouseY<=topPos+configOffsetTop+16) {configCycle(2);playClickSound(0.5f);
                Channel.sendToServer(new BatteryConfigToServer(config,getMenu().getPos()));}
            else if(pMouseY>topPos+configOffsetTop-18 && pMouseY<=topPos+configOffsetTop-18+16) {configCycle(1);playClickSound(0.5f);
                Channel.sendToServer(new BatteryConfigToServer(config,getMenu().getPos()));}
            ...
            configCycle(5); playClickSound(0.5f);
            Channel.sendToServer(new BatteryConfigToServer(config,getMenu().getPos()));
        }
